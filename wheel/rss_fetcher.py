...
def fetch_category_rss(category_id: int) -> str:
    ...
    for feed in feeds:
        ...
        for entry in parsed_feed.entries[:5]:  # Limit to 5 entries
            if hasattr(entry,
                       'published_parsed') and entry.published_parsed.tm_year == time.localtime().tm_year and entry.published_parsed.tm_mon == time.localtime().tm_mon and entry.published_parsed.tm_mday == time.localtime().tm_mday:
                print(f"Entry: {entry.title}")
                scraped_content = scrape_article(entry.link)
                # print(f"Scraped content for {entry.title}: {scraped_content}")
                if scraped_content:
